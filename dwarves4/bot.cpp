...
{	
	...
	else	//now there is enough limber
	{
		if(day < 7) //NOT DAY 7
		{
			...
			if(isfenceUp[dwarf.name()] == true) //when fence is up collect apples
			{
				dwarf.start_pick(directionPickApple(dwarf, r, c));
			}
		}//if not day 7 
		...
		{	
			...
			{
				if(ifDay(hours, minutes) == true && isfenceUp[dwarf.name()] == true)//daytime on day 7
				{
				...
				if(ifDay(hours, minutes) == true && isfenceUp[dwarf.name()] == false) //fences are down
				{
					...
					if(dwarf.name() == 1 && ifmoved[dwarf.name()] == false)
					{
					...
					if(dwarf.name() == 2 && ifmoved[dwarf.name()] == false)
					{
							ifmoved[dwarf.name()] = true;
							log << "Walk to " << 0 << " " << 0 << endl;
					...
					if(dwarf.name() == 1 && ifmoved[dwarf.name()] == true)
					{
					...
					if(dwarf.name() == 2 && ifmoved[dwarf.name()] == true)
					{
						if(ifBuild[dwarf.name()] == false)	//build to the west
						{
							dwarf.start_build(WEST);
			...
			if(hours > 20)
			{
				log << dwarf.name() << "PROTECT" << endl;
				//build all around				
				if(dwarf.look(dwarf.row(), dwarf.col()+1) == EMPTY)
				{
					log << dwarf.name() << "EAST" << endl;
					dwarf.start_build(EAST);
				}
			
				else if(dwarf.look(dwarf.row(), dwarf.col()-1) == EMPTY) 
				{
					log << dwarf.name() << "WEST" << endl;
					dwarf.start_build(WEST);
				}
			
				else if(dwarf.look(dwarf.row()+1, dwarf.col()) == EMPTY)
				{
					log << dwarf.name() << "SOUTH" << endl;
					dwarf.start_build(SOUTH);
				}
			
				else if(dwarf.look(dwarf.row()-1, dwarf.col()) == EMPTY)
				{
					log << dwarf.name() << "NORTH" << endl;
					dwarf.start_build(NORTH);
				}
